...
async fn list_lambda_functions(state: State<'_, AppState>) -> Result<Vec<LambdaFunction>, String> {
    ...
    let client = guard.as_ref().ok_or_else(|| String::from("Not logged in. Please login first."))?;

    let resp = client.list_functions()
        .send()
        .await
...
#[tauri::command]
async fn list_function_logs(log_group_name: &str, state: State<'_, AppState>) -> Result<Vec<LogEvent>, String> {
    let guard = state.cloudwatch_client.lock().await;
    let client = guard.as_ref().ok_or_else(|| "Not logged in. Please login first.")?;
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("failed to get log events for log group named {}: {}", log_group_name, e))?;

    let events = resp.events().iter().map(|event| {
        LogEvent {
...
async fn login_aws_profile(profile: &str, state: State<'_, AppState>) -> Result<String, String> {
    ...
    if !output.status.success() {
        return Err(format!("Login failed {}", String::from_utf8_lossy(&output.stderr)));
    }
...
fn list_aws_profiles() -> Result<Vec<String>, String> {
    ...
    let cfg_path = format!("{}/.aws/config", home);

    let content = std::fs::read_to_string(cfg_path.clone())
        .map_err(|e| format!("Reading config file {} failed with {}", cfg_path, e))?;

    let mut profiles = Vec::new();
    for line in content.lines() {
        if line.trim().starts_with("[profile ") && line.contains("]") {
            let start = line.find("[profile ").unwrap() + 8;
